...
class AbstractDataset:
  ...
  def _leave_one_out(self) -> dict[str, datasets_lib.Dataset]:
    ...
    Standard leave-one-out protocol (per user):
    - Train: sequence[:-2] predict sequence[-2]
    - Val: sequence[:-1] predict sequence[-1] (for hyperparameter tuning)
    - Test: sequence[:-1] predict sequence[-1] (for final evaluation, same task as val)

    The key insight: val and test do the SAME task but serve different purposes.
    Val is for hyperparameter selection, test is for final performance reporting.
  ...
  def log(self, message: str, level: str = 'info') -> None:
    ...
    from genrec.utils_log import log as log_lib
    return log_lib(
