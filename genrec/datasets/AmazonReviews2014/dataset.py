...
#             except json.JSONDecodeError:
#                 pass
#             # 2) Python 字面量（支持单引号、u'...'、True/False/None）
#             try:
...
#             except Exception:
#                 pass
#             # 3) 最后兜底：替换特殊标记并转为 JSON
#             relaxed = (l.replace("u'", "'")
#                          .replace('None', 'null')
#                          .replace('true', 'True')
#                          .replace('false', 'False'))
#             try:
#                 yield json.loads(relaxed.replace("'", '"'))
#             except Exception as e:
#                 # 打印片段，便于定位脏行
#                 preview = relaxed[:200].replace('\n', ' ')
#                 raise ValueError(f'无法解析元数据行: {preview}...') from e
...
class AmazonReviews2014(AbstractDataset):
  ...
  def _load_metadata(
    ...
    for info in tqdm(parse_gz(path)):
  ...
  def _extract_meta_sentences(self, metadata: dict[str, Any]) -> dict[str, str]:
    ...
    item2meta = {}
    for item, meta in tqdm(metadata.items(),total=len(metadata), desc='Extracting meta sentences'):
      meta_sentence = ''
      keys = set(meta.keys())
      features_needed = [
          'title',
