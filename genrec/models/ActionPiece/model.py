...
class ActionPiece(AbstractModel):
  ...
  def __init__(self, config, dataset, tokenizer):
    ...
    self.ranking_temperature = config['ranking_temperature']
  ...
  def forward(self, batch: dict[Any, Any]) -> torch.Tensor:
    ...
    if 'decoder_input_ids' not in batch:
      ...
      hf_inputs = {k: v for k, v in batch.items() if k != 'labels'}
      outputs = self.t5(**hf_inputs, return_dict=True)

    logits = outputs.logits  # (B, L, V)
    
    # Ranking-guided generation loss with temperature scaling
    # 论文公式: P_θ(y_t|y_{<t},x) = exp(p(y_t)/τ) / Σ_{v∈V} exp(p(v)/τ)
    # 通过在 cross_entropy 之前缩放 logits，实现温度调节
    # τ < 1: 增强对 hard negatives 的惩罚（更陡峭的分布）
    # τ = 1: 等价于标准交叉熵
    # τ > 1: 平滑分布
    tau = self.ranking_temperature
    scaled_logits = logits / tau
    
    # 计算排名引导的交叉熵损失
    # F.cross_entropy 内部会计算 log_softmax，因此这等价于论文公式
    loss = F.cross_entropy(
      scaled_logits.view(-1, scaled_logits.size(-1)),
      labels.view(-1),
      ignore_index=-100,
      reduction='mean'  # 显式指定 reduction 方式
    )
  ...
  def beam_search_step(
    ...
    # Apply temperature scaling for ranking-guided inference
    # This ensures consistency between training and inference
    scaled_logits = next_token_logits / self.ranking_temperature # add
