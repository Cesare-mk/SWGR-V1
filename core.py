...
class ActionPieceCore:
    ...
    def _compute_pair_priority(self, tk1: int, tk2: int, cnt: float) -> float:
        ...
        #  输入裁剪：限制权重范围，防止极端值
        log_w1 = np.clip(log_w1, -4.0, 4.0)  # w ∈ [0.018, 54.6]
        log_w2 = np.clip(log_w2, -4.0, 4.0)
        ...
        # 输出裁剪：防止极端优先级
        log_weight_component = np.clip(log_weight_component, -3.0, 3.0)

        log_priority = log_cnt + log_weight_component
        priority = np.exp(log_priority)
        
        # 最终安全保护
        return np.clip(priority, 1e-8, 1e8)
    ...
    def analyze_weight_effectiveness(self):
        ...
        extreme_weights = sum(1 for w in all_weight_factors if w > 2.0 or w < 0.5)
        ...
        if neutral_ratio > 0.8:
            ...
            self.logger.info(f"  建议：增大γ参数 (当前γ={self.gamma:.1f} → 建议γ={self.gamma*1.5:.1f}) 或检查权重文件质量")
        elif extreme_ratio > 0.3:
            self.logger.info(f"  ⚠ 权重过度极化：{extreme_ratio:.1%}的权重因子过于极端")
            self.logger.info(f"  建议：减小γ参数 (当前γ={self.gamma:.1f} → 建议γ={self.gamma*0.7:.1f})")
        elif weight_std < 0.1:
            self.logger.info(f"  ⚠ 权重变异性不足：权重因子标准差={weight_std:.3f}")
            self.logger.info(f"  建议：增大γ参数或检查权重计算逻辑")
        elif overlap_counts.get(20, 1.0) > 0.9:
            self.logger.info(f"  ⚠ 排序影响有限：Top-20重叠度={overlap_counts.get(20, 1.0):.1%}")
            self.logger.info(f"  建议：权重可能需要更强的区分度，考虑增大γ")
        else:
